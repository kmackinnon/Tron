...
public abstract class DatabaseInterface {
    ...
    /**
     * Confirms that a given password is correct for the user corresponding
     * to the given id. If the password is correct, it returns true, otherwise
     * it returns false.
     * @param uid User ID
    ...
    /**
     * Updates a given User's Statistics, providing the uid is valid,
     * and the feature is supported. If the uid is invalid, nothing happens.
     * If the feature is not supported, an UnsupportedOperationException is thrown.
     * @param uid User ID
    ...
    /**
     * Checks if a user exists or not. That is, it checks if a username is not unique.
    ...
    /**
     * Adds a record of a finished game to the Implementation's data storage medium.
     * If Implementation does not support Game record saving this method throws an
    ...
    /**
     * Retrieves Statistics for a given user. If ID is invalid, returns null.
     * If Implementation doesn't support Statistics, throws UnsupportedOperationException.
     * @param uid User ID
    ...
    /**
     * Adds new map to the database. Name must be unique, otherwise method silently fails.
     * If Implementation does not support storing and loading of maps, throws UnsupportedOperationException.
    ...
    /**
     * Retrieves map of given name from storage.
     * If map isn't found returns null.
     * If Implementation does not support storing and loading of maps,
     * throws UnsupportedOperationException.
    ...
    /**
     * Retrieves the Top ten players by wins.
     * Outputs as a 2d array of strings.
     * Returned string must be 2*10.
     * 1st column is the username, 2nd column is the number of wins.
     * Each Row is a separate player.
    ...
    /**
     * Retrieves comparative wins between two users.
     * returned value will be an array of ints of length 2.
     * val[0] = Player One's Win count against Player Two.
     * val[1] = Player Two's Win count against Player One.
